...
### 📋 출퇴근 관리
- **채널 기반 관리**: 팀 채널에서 모든 구성원이 함께 출퇴근 관리
- **개인별 추적**: 각 사용자의 출퇴근 기록은 개별적으로 관리
- **원클릭 출퇴근**: 버튼 클릭으로 간편하게 출퇴근 기록
...
- **작동 방식**: 
  - 오늘 날짜의 모든 태스크에 퇴근 코멘트 추가
  - 새로운 태스크를 생성하지 않고 기존 태스크에 코멘트로 기록
  - 각 태스크마다 동일한 퇴근 정보 코멘트 추가
- **코멘트 내용**:
  - 퇴근 시간 및 작성자
  - 오늘 작업 내역 및 시간
  - 일일/월 누적 작업시간
...
### 채널에서 사용 (권장)
1. 출퇴근 관리용 채널에 봇 초대
2. 봇을 멘션하여 출퇴근 버튼 표시: `@출퇴근봇`
3. 각 사용자별로 개별 버튼이 표시되며, 클릭하여 출퇴근 기록
4. 모든 채널 참가자가 동일한 채널에서 각자의 출퇴근 관리 가능

### DM에서 사용
1. 봇과의 DM 채널에서 직접 출퇴근 관리
2. 개인용 출퇴근 버튼 사용
...
### 사용 시나리오
- **팀 채널**: 팀원들이 한 채널에서 함께 출퇴근 관리
- **개인 DM**: 개인적으로 조용히 출퇴근 관리
- **태스크 선택**: 출퇴근 시 Notion 태스크 선택하여 작업 기록
